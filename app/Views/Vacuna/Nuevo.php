        ...
        <div class="container">                              
            ...
            <div class="row">
                <div class="col-md-4 col-sm-12 col-xs-12">
                    <div class="mb-3">                        
                        <input class="form-control" name="vacuna" type="text" id="vacuna" maxlength="50" placeholder="Vacuna" autofocus >
                    </div>
                </div>
...
<script>
    ...
    function enviarPorAjax(){
        ...
        xhr.open('POST', baseURL +'mantenedor/vacuna/agregar', true);
        xhr.onreadystatechange = function(){
            if (xhr.readyState == 4 && xhr.status == 200){ 
                console.log(xhr.responseText);
                json=JSON.parse(xhr.responseText)
                if (json['respuesta']=="ok"){
                    swal('Mantenedor de Vacuna','Registro agregado con éxito','success')
                }else{
                    if (json['respuesta']=="no"){
                        swal('Mantenedor de Vacuna','Registro no se pudo agregar','error')
                    }else{
                        if (json['respuesta']=="blanco"){
                            swal('Mantenedor de Vacuna','No se permite espacios en blanco','error')
                        }else{
                            if (json['respuesta']=="existe"){
                                swal('Mantenedor de Vacuna','Vacuna ya existe en la base de datos','error')
                            }
                        }
                    }
                }
