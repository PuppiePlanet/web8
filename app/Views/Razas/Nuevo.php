    <form id="myForm">
        <div class="container">                              
            ...
            <div class="row">
                <div class="col-12">
                    <div class="mb-3">
                        <input class="form-control" name="raza" type="text" id="raza" maxlength="200" placeholder="Raza" >
                    </div>
                </div>
            </div>
            <div class="row">
...
<script>
    ...
    function enviarPorAjax(){
        ...
        xhr.open('POST', baseURL +'mantenedor/razas/agregar', true);
        xhr.onreadystatechange = function(){
            if (xhr.readyState == 4 && xhr.status == 200){ 
                console.log(xhr.responseText); 
                json=JSON.parse(xhr.responseText)
                if (json['respuesta']=="ok"){
                    swal('Mantenedor de Razas','Registro agregado con éxito','success')
                }else{
                    if (json['respuesta']=="no"){
                        swal('Mantenedor de Razas','Registro no se pudo agregar','error')
                    }else{
                        if (json['respuesta']=="blanco"){
                            swal('Mantenedor de Razas','No se permite espacios en blanco','error')
                        }else{
                            if (json['respuesta']=="existe"){
                                swal('Mantenedor de Razas','Raza ya existe en la base de datos','error')
                            }
                        }
                    }
                }
