...
<script>
        ...
        function enviarPorAjax(){
            ...
            if (inputFile) {
                ...
                xhr.open("POST", "<?php echo site_url('mascotas/subir_archivo'); ?>", true);
                xhr.onreadystatechange = function(){
                    if (xhr.readyState == 4 && xhr.status == 200){ 
                        console.log(xhr.responseText); 
                        json=JSON.parse(xhr.responseText)
                        if (json['respuesta']=="ok"){
                            swal('Postear','Registro agregado con éxito','success')
                        }else{
                ...
                xhr.send(formData);
            }else{swal('Postear','Seleccione un archivo para subir','error')}
        }        
    </script>
